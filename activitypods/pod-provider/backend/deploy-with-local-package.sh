...
# ActivityPods + Atproto Bridge Deployment Script (with local package)
# Usage: ./deploy-with-local-package.sh your-domain.com your-email@example.com
...
if [ "$DOMAIN" = "your-domain.com" ] || [ "$EMAIL" = "your-email@example.com" ]; then
    echo "Usage: ./deploy-with-local-package.sh your-domain.com your-email@example.com"
    echo "Example: ./deploy-with-local-package.sh activitypods.example.com admin@example.com"
    exit 1
fi

echo "🚀 Deploying ActivityPods + Atproto Bridge (with local package) to $DOMAIN"
echo "📧 Email: $EMAIL"
echo ""

# Check if we're in the right directory
if [ ! -f "package.json" ] || [ ! -d "../../semapps" ]; then
    echo "❌ Error: This script must be run from the backend directory with semapps in the parent directory"
    echo "Expected structure:"
    echo "  activitypods-bridge/"
    echo "  ├── activitypods/pod-provider/backend/  <- Run from here"
    echo "  └── semapps/src/middleware/packages/atproto/"
    exit 1
fi
...
echo "🔍 Verifying local atproto package..."
if [ ! -d "../../semapps/src/middleware/packages/atproto" ]; then
    echo "❌ Error: Local atproto package not found at ../../semapps/src/middleware/packages/atproto"
    exit 1
fi

echo "✅ Local atproto package found"

# Build and start services
echo "🐳 Building and starting services..."
echo "Note: This may take several minutes as it builds the entire repository..."
